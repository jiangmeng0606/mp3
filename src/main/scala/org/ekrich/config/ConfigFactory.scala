...
import org.ekrich.config.impl.{ConfigImpl, Parseable}
 ...
 *
 * See also [[ConfigValueFactory]] which contains static methods for converting
 * Java values into a [[ConfigObject]]. You can then convert a [[ConfigObject]]
 * into a [[Config]] with [[ConfigObject#toConfig ConfigObject.toConfig()]].
 *
 ...
 * [[https://github.com/ekrich/sconfig/tree/master/examples on GitHub]]. Also be
...
object ConfigFactory extends PlatformConfigFactory {
   ...
   * package, so it's like `ClassLoader.getResource` not `Class.getResource`).
   *
   ...
   * by substitutions. Substitutions are the `\${foo.bar}` syntax. If you want
   * to parse additional files or something then you need to use
   * [[#load(config:org\.ekrich\.config\.Config)* load(Config)]].
   *
   * To load a standalone resource (without the default reference and default
   * overrides), use
   * [[#parseResourcesAnySyntax(resourceBasename:String)* parseResourcesAnySyntax(String)]]
   * rather than this method. To load only the reference config use
   * [[#defaultReference()* defaultReference()]] and to load only the overrides
   * use [[#defaultOverrides()* defaultOverrides()]].
   *
  ...
  /**
   * Like [[#load(resourceBasename:String)* ConfigFactory.load(String)]] but
   * uses the supplied class loader instead of the current thread's context
   * class loader.
   *
   * To load a standalone resource (without the default reference and default
   * overrides), use
   * [[#parseResourcesAnySyntax(loader:ClassLoader,resourceBasename:String)* parseResourcesAnySyntax(ClassLoader,String)]]
   * rather than this method. To load only the reference config use
   * [[#defaultReference(loader:ClassLoader)* defaultReference(ClassLoader)]]
   * and to load only the overrides use
   * [[#defaultOverrides(loader:ClassLoader)* defaultOverrides(ClassLoader)]].
   *
   * @param loader
  ...
  /**
   * Like [[#load(resourceBasename:String)* ConfigFactory.load(String)]] but
   * allows you to specify parse and resolve options.
   *
   * @param resourceBasename
  ...
  /**
   * Like
   * [[#load(resourceBasename:String,parseOptions:org\.ekrich\.config\.ConfigParseOptions,resolveOptions:org\.ekrich\.config\.ConfigResolveOptions)* load(String,ConfigParseOptions,ConfigResolveOptions)]]
   * but has a class loader parameter that overrides any from the
   * [[ConfigParseOptions]].
   *
   * @param loader
   *   class loader in which to find resources (overrides loader in parse
   *   options)
   * @param resourceBasename
   *   the classpath resource name with optional extension
   * @param parseOptions
  ...
  private def checkedContextClassLoader(methodName: String) = {
    ...
    if (loader == null)
      throw new ConfigException.BugOrBroken(
        "Context class loader is not set for the current thread; " +
  ...
  /**
   * Assembles a standard configuration using a custom `Config` object rather
   * than loading "application.conf". The `Config` object will be sandwiched
  ...
  /**
   * Like [[#load(config:org\.ekrich\.config\.Config)* load(Config)]] but allows
   * you to specify the class loader for looking up resources.
   *
   * @param loader
  ...
  /**
   * Like [[#load(config:org\.ekrich\.config\.Config)* load(Config)]] but allows
   * you to specify [[ConfigResolveOptions]].
   *
   * @param config
   *   the application's portion of the configuration
   * @param resolveOptions
  ...
  /**
   * Like
   * [[#load(config:org\.ekrich\.config\.Config,resolveOptions:org\.ekrich\.config\.ConfigResolveOptions)* load(Config,ConfigResolveOptions)]]
   * but allows you to specify a class loader other than the context class
   * loader.
   *
   * @param loader
   ...
   * different one.
   *
   * This method may return a cached singleton so will not see changes to system
   * properties or config files. (Use [[#invalidateCaches invalidateCaches()]]
  ...
  /**
   * Like [[#load()* load()]] but allows specifying parse options.
   *
   * @param parseOptions
  ...
  /**
   * Like [[#load()* load()]] but allows specifying a class loader other than
   * the thread's current context class loader.
   *
   * @param loader
  ...
  /**
   * Like [[#load()* load()]] but allows specifying a class loader other than
   * the thread's current context class loader and also specify parse options.
   *
   * @param loader
  ...
  /**
   * Like [[#load()* load()]] but allows specifying a class loader other than
   * the thread's current context class loader and also specify resolve options.
   *
   * @param loader
   *   class loader for finding resources
   * @param resolveOptions
  ...
  /**
   * Like [[#load()* load()]] but allows specifying a class loader other than
   * the thread's current context class loader, parse options, and resolve
   * options.
   *
   * @param loader
   *   class loader for finding resources (overrides any loader in parseOptions)
  ...
  /**
   * Like [[#load()* load()]] but allows specifying parse options and resolve
   * options.
   *
   * @param parseOptions
   *   Options for parsing resources
  ...
  /**
   * Obtains the default reference configuration, which is currently created by
   * merging all resources "reference.conf" found on the classpath and
   ...
   * [[#defaultReference(loader:ClassLoader)* defaultReference(ClassLoader)]] if
   * the context class loader is not suitable.
   *
   * The [[#load()* load()]] methods merge this configuration for you
   * automatically.
   *
   * Future versions may look for reference configuration in more places. It is
   * not guaranteed that this method ''only'' looks at "reference.conf".
   *
   * @return
  ...
  /**
   * Like [[#defaultReference()* defaultReference()]] but allows you to specify
   * a class loader to use rather than the current context class loader.
   *
   * @param loader
   *   class loader to look for resources in
   * @return
   ...
   * automatically.
   *
   * Future versions may get overrides in more places. It is not guaranteed that
   * this method <em>only</em> uses system properties.
   *
   * @return
   ...
   * parsing `application.conf`, `application.json`, and
   * `application.properties` on the classpath, but can also be rerouted using
   * the `config.file`, `config.resource`, and `config.url` system properties.
   *
   * The no-arguments [[#load()* load()]] method automatically stacks the
   * [[#defaultReference()* defaultReference()]],
   * [[#defaultApplication()* defaultApplication()]], and
   * [[#defaultOverrides()* defaultOverrides()]] configs. You would use
   * `defaultApplication()` directly only if you're somehow customizing behavior
   * by reimplementing `load()`.
   *
   * The configuration returned by `defaultApplication()` will not be resolved
   * already, in contrast to `defaultReference()` and `defaultOverrides()`. This
   ...
   * `application.{conf,json,properties]]` classpath resources. These system
   * properties should not be set in code (after all, you can just parse
   * whatever you want manually and then use
   * [[#load(config:org\.ekrich\.config\.Config)* load(Config)]] if you don't
   * want to use `application.conf`). The properties are intended for use by the
   * person or script launching the application. For example someone might have
   * a `production.conf` that include `application.conf` but then change a
  ...
  /**
   * Like [[#defaultApplication()* defaultApplication()]] but allows you to
   * specify a class loader to use rather than the current context class loader.
   *
   * @since 1.3.0
   * @param loader
   *   class loader to look for resources in
   ...
   * example. Because a [[Config]] is immutable, anyone with a reference to the
   * old configs will still have the same outdated objects. However, new calls
   * to [[#load()* load()]] or [[#defaultOverrides()* defaultOverrides()]] or
   * [[#defaultReference()* defaultReference()]] may return a new object.
   *
   * This method is primarily intended for use in unit tests, for example, that
   * may want to update a system property then confirm that it's used correctly.
   * In many cases, use of this method may indicate there's a better way to set
   * up your code.
   *
   * Caches may be reloaded immediately or lazily; once you call this method,
   * the reload can occur at any time, even during the invalidation process. So
   * FIRST make the changes you'd like the caches to notice, then SECOND call
   * this method to invalidate caches. Don't expect that invalidating, making
   * changes, then calling [[#load()* load()]], will work. Make changes before
   * you invalidate.
   */
  ...
  /**
   * Gets an empty configuration. See also
   * [[#empty(originDescription:String)* empty(String)]] to create an empty
   * configuration with a description, which may improve user-visible error
   * messages.
   *
   * @return
   ...
   * [[ConfigOrigin]] for this `Config`. The description should be very short
   ...
   * [[ConfigFactory$.parseProperties(properties:java\.util\.Properties)* parseProperties(Properties)]].
   *
   * This method can return a global immutable singleton, so it's preferred over
   * parsing system properties yourself.
   *
   * [[#load()* ConfigFactory.load()]] will include the system properties as
   * overrides already, as will [[#defaultReference()* defaultReference()]] and
   ...
   * [[#invalidateCaches invalidateCaches()]] to force the singleton to reload
   * if you modify system properties.
   *
   * @return
   ...
   * method can return a global immutable singleton.
   *
   ...
   * `java.lang.System.getenv(String)` though, since you can use methods such as
   * [[Config.getInt]].
   *
   * @return
  ...
  /**
   * Converts a Java `java.util.Properties` object to a [[ConfigObject]] using
   * the rules documented in the
   * [[https://github.com/ekrich/sconfig/blob/master/HOCON.md HOCON spec]] The
   ...
   * dropped.
   *
   * If you want to have `System.getProperties()` as a ConfigObject, it's better
   ...
   * [[#parseURL(url:java\.net\.URL,options:org\.ekrich\.config\.ConfigParseOptions)* ConfigFactory.parseURL(URL,ConfigParseOptions)]]
   ...
   * [[Config!.resolve()* Config.resolve()]] or merge the file with any other
   * configuration; this method parses a single file and does nothing else. It
   * does process "include" statements in the parsed file, and may end up doing
   * other IO due to those statements.
   *
   * @param file
   *   the file to parse
   * @param options
  ...
  /**
   * Parses a file with a flexible extension. If the `fileBasename` already ends
   * in a known extension, this method parses it according to that extension
   * (the file's syntax must match its extension). If the `fileBasename` does
   ...
   *
   * If `options` forces a specific syntax, this method only parses files with
   * an extension matching that syntax.
   *
   * If [[ConfigParseOptions#getAllowMissing]] is true, then no files have to
   * exist; if false, then at least one file has to exist.
   *
   * @param fileBasename
  ...
  /**
   * Like
   * [[#parseFileAnySyntax(fileBasename:java\.io\.File,options:org\.ekrich\.config\.ConfigParseOptions)* parseFileAnySyntax(File,ConfigParseOptions)]]
   ...
   * earlier from `ClassLoader.getResources` fall back to (have higher priority
   ...
   * [[#parseFileAnySyntax(fileBasename:java\.io\.File,options:org\.ekrich\.config\.ConfigParseOptions)* parseFileAnySyntax(File,ConfigParseOptions)]]
   * but for classpath resources instead, as in
   * [[#parseResources(resource:String)* parseResources(String)]].
   *
   * There is a thorny problem with this method, which is that
   * `java.lang.ClassLoader.getResources` must be called separately for each
   ...
   *
   * @param klass
   *   class which determines the `ClassLoader` and the package for relative
   *   resource names
   * @param resourceBasename
   *   a resource name as in `java.lang.Class.getResource`, with or without
   *   extension
   * @param options
  ...
  /**
   * Like
   * [[#parseResourcesAnySyntax(klass:Class[_],resourceBasename:String,options:org\.ekrich\.config\.ConfigParseOptions)* parseResourcesAnySyntax(Class,String,ConfigParseOptions)]]
   * but always uses default parse options.
   *
   * @param klass
   *   `klass.getClassLoader()` will be used to load resources, and non-absolute
   ...
   * `java.lang.Class.getResource`, so the name never begins with a slash.
   *
   * See
   * [[#parseResources(klass:Class[_],resource:String,options:org\.ekrich\.config\.ConfigParseOptions)* parseResources(Class,String,ConfigParseOptions)]]
   * for full details.
   *
   * @param loader
   *   will be used to load resources by setting this loader on the provided
   *   options
   * @param resource
   *   resource to look up
   * @param options
   ...
   *
   * [[#parseResourcesAnySyntax(klass:Class[_],resourceBasename:String,options:org\.ekrich\.config\.ConfigParseOptions)* parseResourcesAnySyntax(Class,String,ConfigParseOptions)]]
   * differs in the syntax for the resource name, but it is useful to review for
   * some details and caveats on this method.
   *
   * @param loader
   *   class loader to look up resources in, will be set on options
   * @param resourceBasename
   *   a resource name as in `java.lang.ClassLoader.getResource`, with or
   *   without extension
   * @param options
  ...
  /**
   * Like
   * [[#parseResourcesAnySyntax(loader:ClassLoader,resourceBasename:String,options:org\.ekrich\.config\.ConfigParseOptions)* parseResourcesAnySyntax(ClassLoader,String,ConfigParseOptions)]]
   * but always uses default parse options.
   *
   * @param loader
   *   will be used to load resources
   ...
   * Like
   * [[#parseResources(loader:ClassLoader,resource:String,options:org\.ekrich\.config\.ConfigParseOptions)* parseResources(ClassLoader,String,ConfigParseOptions)]]
   * but uses thread's current context class loader if none is set in the
   * ConfigParseOptions.
   *
   * @param resource
   *   the resource name
   ...
   * object of "b". The caller of this method should ensure that doesn't happen.
   *
   * @param values
   *   map from paths to plain Java objects
   * @param originDescription
   *   description of what this map represents, like a filename, or "default
   *   settings" (origin description is used in error messages)
   * @return
